...
function Term({ children }: { children: React.ReactNode }) {
  return (
    <View style={styles.term}>
      <Check size={14} color="#2f9f92" strokeWidth={2} style={{ marginTop: 5 }} />
      <T size={13} color="#5f5870" lineHeight={24} style={{ flex: 1 }}>{children}</T>
    </View>
  );
}
...
function Cell({ label, value, hint }: { label: string; value: string; hint: string }) {
  return (
    <View style={{ flex: 1, minWidth: '45%' }}>
      <T size={12} color="#746d7d" style={{ marginBottom: 6 }}>{label}</T>
      <T weight="semibold" size={18} lineHeight={27} tabular>{value}</T>
      <T size={11} color="#756e7c" lineHeight={16} style={{ marginTop: 5 }}>{hint}</T>
    </View>
  );
}
    ...
    <View>
      <View style={styles.heading}>
        ...
        <View style={[styles.status, sealed && styles.statusSealed]}>
          {sealed ? <ShieldCheck size={13} color="#24776b" strokeWidth={2} /> : <FileText size={13} color={colors.statusText} strokeWidth={2} />}
          <T size={9} lineHeight={12} color={sealed ? '#24776b' : colors.statusText}>{sealed ? 'Sealed' : 'Awaiting'}</T>
        </View>
      </View>

      {sealed && (
        <View style={styles.seal}>
          <View style={styles.sealIcon}><CheckCheck size={26} color="#d34f89" strokeWidth={1.8} /></View>
          <View style={{ flex: 1 }}>
            <T weight="bold" size={14} color={colors.sealText} lineHeight={22}>Both signatures verified</T>
            <T size={11} color="#55756f" lineHeight={19} style={{ marginTop: 3 }}>Changes to these signed terms fail verification.</T>
          </View>
        </View>
      )}
      ...
      <View style={styles.document}>
        <View style={styles.documentTop}>
          <T size={10} color="#7a7282">Trade cost agreement</T>
          <T mono size={10} color="#7a7282">{snapshot.id}</T>
        </View>
        <T serif size={26} lineHeight={36} style={{ marginTop: 17, letterSpacing: -0.6 }}>{snapshot.invoice.goods}</T>
        <T size={11} color="#7a7282" style={{ marginTop: 10 }}>{names.buyer}  ↔  {names.supplier}</T>
        ...
        <View style={styles.terms}>
          <Term><T weight="bold" size={13} color="#5f5870">{buyerCovers ? 'Bilal (buyer)' : 'Amira (supplier)'}</T> covers downstream fees, estimated at {range(snapshot.quote.downstreamFeeEur.min, snapshot.quote.downstreamFeeEur.max, 'EUR')}.</Term>
          <Term>Payment due <T weight="bold" size={13} color="#5f5870">{snapshot.invoice.dueDate}</T>. Delivery: {snapshot.terms.deliveryWindow}.</Term>
        ...
        <Disclosure label="Rate, fee assumptions & record details" size={12} weight="regular" style={{ marginTop: 6 }} contentStyle={{ gap: 10, paddingBottom: 8 }}>
          <T size={11} color="#766f7e" lineHeight={21}>Reference: {snapshot.quote.referenceRate.toFixed(4)} CAD/EUR. Customer rate: {snapshot.quote.rateCadPerEur.toFixed(5)} CAD/EUR. Transfer fee: {cad(snapshot.quote.transferFeeCad)}.</T>
          <T size={11} color="#766f7e" lineHeight={21}>{snapshot.quote.source} · {snapshot.quote.asOf}</T>
          <T size={11} color="#766f7e" lineHeight={21}>Created {new Date(snapshot.createdAt).toLocaleString()}. The displayed signature times come from this device.</T>
          <T size={11} color="#766f7e" lineHeight={21}>{snapshot.disclosure}</T>
        </Disclosure>
      </View>
          ...
          return (
            <View key={party} style={styles.slot} accessibilityLabel={`${party} signature: ${signature ? 'signed' : 'pending'}`}>
              <View style={styles.slotLabel}>
                <T size={10} color="#7a7282">{party === 'buyer' ? 'Buyer' : 'Supplier'}</T>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 3 }}>
                  {signature && <Check size={12} color="#2f9f92" strokeWidth={2.2} />}
                  <T size={10} color={signature ? '#2f9f92' : '#7a7282'}>{signature ? 'Signed' : 'Pending'}</T>
                </View>
              </View>
              {signature
                ? <T serif size={18} lineHeight={26} color="#5d427f" style={{ minHeight: 40 }}>{names[party]}</T>
                : <T weight="medium" size={13} lineHeight={21} style={{ minHeight: 40 }}>{names[party]}</T>}
              <T size={10} color="#766f7e" style={{ marginTop: 8 }}>{signature ? `Verified · ${shortTime(signature.signedAt)}` : 'Ready to review and sign'}</T>
            </View>
          );
        })}
      </View>
          ...
          {selectedSigned ? (
            <>
              <View style={styles.signedWait}>
                <Check size={18} color="#55756f" strokeWidth={2} style={{ marginTop: 3 }} />
                <T size={14} color="#55756f" lineHeight={25} style={{ flex: 1 }}>Your signature is recorded. The other party can now review.</T>
              </View>
              <PrimaryButton label={`Review as ${firstName(otherParty(role))}`} icon={ArrowRight} onPress={() => setRole(otherParty(role))} fullWidth />
              <T size={11} color="#7a7282" lineHeight={21} center style={{ marginTop: 10 }}>Switching fictional roles on this device for the demo.</T>
            </>
          ) : (
            <>
              <View style={styles.signingAs}>
                <Users size={15} color="#716879" strokeWidth={1.8} />
                <T size={13} color="#716879">You’re reviewing as <T weight="bold" size={13} color="#4d3b75">{names[role]}</T></T>
              ...
              <Pressable accessibilityRole="checkbox" accessibilityState={{ checked: consent }} onPress={() => setConsent(!consent)} style={styles.consent}>
                <Checkbox value={consent} onValueChange={setConsent} color={consent ? '#6750a8' : undefined} style={styles.checkbox} />
                <T size={14} color="#615970" lineHeight={25} style={{ flex: 1 }}>I agree to these terms and understand the costs are illustrative estimates, not a payment or rate guarantee.</T>
              </Pressable>
              ...
              <T size={11} color="#7a7282" lineHeight={21} center style={{ marginTop: 10 }}>Real cryptographic signature · fictional, unverified identity</T>
            </>
          )}
        </View>
      ) : (
      ...
      <Pressable accessibilityRole="button" accessibilityLabel="Copy content fingerprint" onPress={() => { void copyHash(); }} style={({ pressed }) => [styles.hash, pressed && { opacity: 0.8 }]}>
        <FingerprintPattern size={16} color="#735989" strokeWidth={1.7} />
        <View style={{ flex: 1 }}>
          <T size={10} color="#735989" style={{ marginBottom: 4 }}>SHA-256 content fingerprint · tap to copy</T>
      ...
      <T size={11} color="#766f7e" lineHeight={21} center style={{ marginTop: 6 }}>Keys and messages stay in memory. Share the record before closing the app.</T>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  statusSealed: { backgroundColor: '#e9f8f3' },
  ...
  sealIcon: { width: 43, height: 43, borderRadius: 22, borderWidth: 1, borderColor: '#b7ddd6', alignItems: 'center', justifyContent: 'center' },
  ...
  terms: { gap: 13, borderTopWidth: 1, borderTopColor: '#ddd3e1', borderStyle: 'dashed', paddingTop: 18 },
  term: { flexDirection: 'row', gap: 8 },
  signatures: { flexDirection: 'row', gap: 14, marginTop: 22 },
  slot: { flex: 1, borderBottomWidth: 1, borderBottomColor: '#ddd2e2', paddingBottom: 16 },
