...
function App() {
  ...
  return <div className={`app-shell screen-${screen}`}>
    <aside className="desktop-nav" aria-label="Workspace navigation">
      <div className="desktop-brand"><img src="./favicon.svg" alt="" /><span>sanad</span></div>
      ...
      <main id="workspace">
        {screen === 'chat' ? <section className="conversation" aria-label="Trade conversation">
          ...
          <div className="chat-timeline" ref={chatTimeline} onScroll={e => { chatScroll.current = e.currentTarget.scrollTop; }}><div className="chat-date">5 September · Sample conversation</div>
            {messages.map((item, index) => <div key={item.id} className={`message-group ${item.from === 'buyer' ? 'outgoing' : 'incoming'}`}><div className="message-bubble">{item.text}<span className="message-time">{item.time}{item.from === 'buyer' && <CheckCheck size={13} />}</span></div>
                {index === 0 && <button className="invoice-attachment" onClick={() => { setEditInvoice({ ...invoice }); setEditErrors([]); setModal('edit'); }} aria-label="View sample invoice"><div className="invoice-file-icon"><FileText size={23} /></div><div><strong>Invoice SF-1048</strong><span>{invoice.quantity} bottles · Organic olive oil</span><b>{eur(invoice.amountEur)}</b></div><ChevronRight size={18} /></button>}
              </div>)}
            ...
            <button className="chat-finance-bridge" onClick={() => goTo('finance')}><span className="bridge-icon">{sealed ? <ShieldCheck size={23} /> : <FileText size={23} />}</span><span><small>Invoice {invoice.id} · {eur(invoice.amountEur)}</small><strong>{sealed ? 'View signed agreement' : agreement ? 'Review shared agreement' : 'Review payment options'}</strong></span><ArrowRight size={20} /></button>
          ...
          <ol className="steps"><li className={!agreement ? 'current' : 'done'}><span>{agreement ? <Check size={13} /> : '1'}</span>Compare</li><li className={agreement && !sealed ? 'current' : sealed ? 'done' : ''}><span>{sealed ? <Check size={13} /> : '2'}</span>Agree</li><li className={sealed ? 'current' : ''}><span>{sealed ? <Check size={13} /> : '3'}</span>Keep record</li></ol>
          ...
          {!agreement ? <>
            ...
            {sealed && <div className="seal-moment"><div className="seal-icon"><CheckCheck size={28} /></div><div><strong>Both signatures verified</strong><span>Changes to these signed terms fail verification.</span></div></div>}
